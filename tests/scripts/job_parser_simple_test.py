"""
Simple Job Parser Test Module

A simplified version of job parser tests that doesn't rely on heavy NLP dependencies.
Uses regex and basic text processing instead of spaCy for testing job parsing features.

Usage:
    python job_parser_simple_test.py
"""
...
# Simple job parser implementations to avoid dependency on spaCy
def simple_extract_job_description(text):
    """
    Extract job description from text using simple regex patterns
    
    Args:
        text (str): Job posting text
        
    Returns:
        str: Extracted job description or original text if no clear sections found
    """
    ...
    for marker in desc_markers:
        match = re.search(marker, clean_text)
        if match:
            start_pos = match.start()
            # Return everything from the marker onwards
            return clean_text[start_pos:].strip()
    
    # If no markers found, return the original cleaned text
    return clean_text.strip()

def simple_extract_job_requirements(job_description):
    """
    Extract job requirements from job description using simple patterns
    
    Args:
        job_description (str): Job description text
        
    Returns:
        list: List of job requirements/skills
    """
    ...
    for pattern in req_section_patterns:
        matches = re.search(pattern, job_description, re.DOTALL)
        if matches:
            section = matches.group(1).strip()
            # Extract bullet points
            bullet_items = re.findall(r'[-•*]\s*([^\n]+)', section)
            requirements.extend(bullet_items)
            
            # If no bullet points, split by newlines
            if not bullet_items:
                requirements.extend([s.strip() for s in section.split('\n') if s.strip()])
    
    # If no structured requirements found, look for common skill keywords
    if not requirements:
        # Extract sentences containing common requirement indicators
        indicators = [
            ...
            r'ability\s+to\s+([^,.]+)',
            r'comfortable\s+(?:with|in)\s+([^,.]+)',
            r'years\s+(?:with|of|in)\s+([^,.]+)',
        ]
        
        for indicator in indicators:
            matches = re.findall(indicator, job_description, re.IGNORECASE)
            requirements.extend([m.strip() for m in matches if m.strip()])
    
    # Remove duplicates and empty strings
    return list(dict.fromkeys([req for req in requirements if req]))
...
def test_extract_job_description():
    ...
    for test_file in test_files:
        ...
        extracted_description = simple_extract_job_description(content)
...
def test_extract_job_requirements():
    ...
    for test_file in test_files:
        ...
        description = simple_extract_job_description(content)
        requirements = simple_extract_job_requirements(description)
...
if __name__ == "__main__":
    try:
        print("Running Simple Job Parser Tests...")
